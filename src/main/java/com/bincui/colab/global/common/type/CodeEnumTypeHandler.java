...
/**
 * {@link CodeEnum}을 구현한 열거형 내의 문자열 타입 코드값과 데이터베이스 컬럼 간의 매핑을 통해
 * {@code code}값을 데이터베이스에 저장하거나 데이터베이스로부터 읽어온 코드값을 열거형으로 변환하는 타입 핸들러
 *
...
public class CodeEnumTypeHandler<E extends Enum<E>&CodeEnum> extends BaseTypeHandler<E> {
    ...
    /**
     * 주어진 코드값과 매치되는 값을 가진 열거형의 상수를 반환한다.
     *
     * @param code  데이터베이스로부터 읽어온 코드값
     * @return      {@code code}와 일치한 값을 가진 열거형 상수
     */
